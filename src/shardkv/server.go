...
// 应用到状态机中
func (kv *ShardKV) applyToStateMachine(op Op, sharId int) *OpReply {
	var value string
	var err Err

	switch op.OpType {
	case OpGet:
		value, err = kv.shards[sharId].Get(op.Key)
	case OpPut:
		err = kv.shards[sharId].Put(op.Key, op.Value)
	case OpAppend:
		err = kv.shards[sharId].Append(op.Key, op.Value)
	}
